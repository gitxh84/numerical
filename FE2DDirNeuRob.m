function uh=FE2DDirNeuRob(nodes,elements,dirichlet,neumann,robin,kappa,hconv,r)
...
for e=1:n_el
    ...
    % directly copied frm FE2DNonzeroDirichlet.m
...
for i=1:size(neumann,1)
    ...
    % directly copied from FE2DDirNeu.m
    for p = 1:2   
...
for i=1:size(robin,1)
    e=round(robin(i,1)); %round() to ensure they are integers
    v1=round(robin(i,2)); v2=round(robin(i,3)); 
    gR1=robin(i,4); gR2=robin(i,5);
    xnod=[nodes(v1,:);nodes(v2,:)];
    medge=hconv*localMassEdge(xnod);
    %... Robin BCs (modifies K and F)
    
    for p = 1:2   
        extra_term = gR1 * medge(p,1) + gR2 * medge(p,2);
        if p == 1
            if ID(v1) > 0
                F(ID(v1)) = F(ID(v1)) + extra_term;
            end
